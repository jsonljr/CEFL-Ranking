...
class SCAServer(Server):
    ...
    def train(self, communication_rounds=10, local_epochs=1):
        ...
        for round in range(communication_rounds):
            ...
            print(f"本轮下行通信开销: {round_comm_bytes_downlink / (1024 ** 2):.6f} MB")
            print(f"本轮上行通信开销: {round_comm_bytes_uplink / (1024 ** 2):.6f} MB")
            print(f"本轮总通信开销: {round_comm_bytes / (1024 ** 2):.6f} MB")
            print(f"本轮模型计算开销: {round_flops_model / 1e9:.4f} GFLOPs")
            print(f"本轮SCA压缩开销: {round_flops_sca} FLOPs")
            print(f"本轮计算总 FLOPs: {round_computation_flops} FLOPs")
            print(f"本轮训练时间: {round_computation_time:.6f} 秒")

...
class QSGDServer(Server):
    ...
    def train(self, communication_rounds=10, local_epochs=1, client_frac=0.4):
        print("Starting FedAvg with QSGD...")
        start_all = time.time()
        for r in range(communication_rounds):
            ...
            print(f"Round {r+1}, selected: {[c.client_id for c in selected_clients]}")
            client_payloads = []
            round_time = 0.0
            round_flops_model = 0
            round_flops_comp = 0
            ...
            round_comm = downlink_bytes + round_uplink
            if client_payloads:
            ...
            self.history['total_comm_bytes'] += int(round_comm)
            self.history['total_computation_flops'] += int(round_flops_model + round_flops_comp)
            self.evaluate()
            print(f"[Round {r+1}] comm: {round_comm/(1024**2):.4f} MB, model FLOPs: {round_flops_model/1e9:.4f} GFLOPs, "
                  f"comp FLOPs: {round_flops_comp} FLOPs, time: {time.time()-round_start:.2f}s")
        self.history['total_training_time'] = time.time() - start_all
        print("Training finished")
        self.print_cost_summary()
        self.save_detailed_history_to_txt(self.history, "QSGD_cifart10Res-10.txt")

...
class DGCServer(Server):
    ...
    def train(self, communication_rounds=10, local_epochs=1):
        ...
        for round in range(communication_rounds):
            ...
            uplink_bytes = len(selected_clients) * uplink_bytes_per_client
            ...
            self.history['total_training_time'] += round_computation_time
            self.evaluate()
            print(f"本轮下行(全模型)开销: {downlink_bytes / (1024 ** 2):.2f} MB")
            print(f"本轮上行(估算DGC压缩)开销: {uplink_bytes / (1024 ** 2):.4f} MB")
        ...
        self.save_detailed_history_to_txt(self.history, "DGC_cifar10Res-10.txt")

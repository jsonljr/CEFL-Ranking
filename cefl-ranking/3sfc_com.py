...
class Client:
    ...
    def compute_synthetic_sample(self, n_sample, n_classes):
        ...
        for _ in range(10):
            def closure():
                optimizer.zero_grad()
                inputs_batch = synthetic_inputs.view(-1, *synthetic_inputs.shape[1:])
...
class Server:
    ...
    def train(self, communication_rounds=10, local_epochs=1, method="fedsynth", n_sample=10, n_classes=10):
        ...
        for round_idx in range(1, communication_rounds + 1):
            ...
            client_network_state = {client.id: np.random.choice(states, p=probs)
            ...
            for client in participating_clients:
                ...
                if method == "fedsynth":
                    inputs, labels, scale, _ = client.compute_fedsynth(
                        n_sample, n_classes, eta_w=0.01, eta=0.01, epochs=local_epochs
                    )
                else:
            ...
            self.aggregate_synthetic_gradients(
                synthetics, scale_factors, [c.dW for c in participating_clients], method=method
            )
            uplink_bytes_total = self.history['communication_cost'][-1]

            for client in participating_clients:
                net_state = client_network_state[client.id]
                speed = NETWORK_SPEED[net_state]
                uplink = uplink_bytes_total / len(participating_clients)
                downlink = model_bytes
                comm_time = (uplink+downlink) / (1024 ** 2) / speed
                round_comm_time += comm_time

                print(f"客户端 {client.id} 网络状态: {net_state}, 速率: {speed} MB/s, "
                      f"通信量: {(uplink+downlink) / (1024 ** 2):.2f} MB, 通信时间: {comm_time:.2f}s")
            ...
            round_time = time.time() - round_start + round_comm_time
...
def main():
    ...
    parser.add_argument('--method', type=str, default='ours', choices=['fedsynth', 'ours'], help='联邦算法')

...
class STCServer:
    ...
    def train(self, communication_rounds=10, local_epochs=1, client_frac=0.5):
        ...
        for r in range(communication_rounds):
            ...
            client_payloads, round_flops = [], 0
            round_comm_bytes, round_comp_time, round_comm_time = 0, 0.0, 0.0
            client_network_state = {client.id: np.random.choice(states, p=probs)
                                    for client in selected_clients}
            uplink_dict = {}
            for client in selected_clients:
                delta, flops, uplink, num_samples = client.local_train(local_epochs)
                client_payloads.append(delta)
                uplink_dict[client.id] = uplink
                round_flops += flops
            ...
            for client in selected_clients:
                ...
                comm_bytes = uplink_dict[client.id] + self.last_downlink_bytes
                ...
                print(f"客户端 {client.id} 网络状态: {net_state}, 速率: {speed} MB/s, "
                      f"通信量: {comm_bytes / (1024 ** 2):.2f} MB, 通信延迟: {comm_time:.2f}s")
            round_total_time = round_comp_time + round_comm_time
            ...
            self.evaluate()
            print(f"本轮计算开销: {round_flops / 1e9:.2f} GFLOPs")
            print(f"本轮训练+通信总时间: {round_total_time:.2f} 秒")
            print(f"本轮总通信量: {round_comm_bytes / (1024 ** 2):.2f} MB, 总通信延迟: {round_comm_time:.2f} 秒")
        end_time = time.time()
        ...
        self.save_detailed_history_to_txt(self.history, "STCcom_mnist20.txt")

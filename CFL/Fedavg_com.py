...
from CFL.model import SimpleCNN,MLP,CIFAR10Net,CIFAR100Net,EMNISTNet,ResNet18
...
class Client:
    ...
    def receive_model(self, global_model):
        ...
        try:
            ...
            self.single_iter_flops = estimate_flops(
                model_cpu, input_size=input_size, batch_size=batch_size, device='cpu'
            )
        except Exception:
            self.single_iter_flops = 0
    ...
    def local_train(self, epochs=1):
        ...
        return delta, training_time, flops_this_round, num_samples

class Server:
    def __init__(self, test_loader, device, global_model=None, num_classes=10):
    ...
    def aggregate(self, client_payloads):
        ...
        if total_samples == 0:
            weights = [1.0 / len(ns)] * len(ns)
        else:
            weights = [n / total_samples for n in ns]

        global_update = {n: torch.zeros_like(p, device=self.device) for n, p in self.global_model.named_parameters() if p.requires_grad}

        for upd, w in zip(updates, weights):
            for k in global_update.keys():
                if k in upd:
                    global_update[k] += upd[k].to(self.device) * w
    ...
    def train(self, communication_rounds=10, local_epochs=1, client_frac=0.4):
        ...
        for r in range(communication_rounds):
            ...
            for client in selected_clients:
                comm_bytes = 2 * self.model_size_bytes
            ...
            print(f"本轮训练+通信总时间: {round_total_time:.2f} 秒")
            print(f"本轮总通信量: {round_comm_bytes / (1024 ** 2):.2f} MB, 总通信时间: {round_comm_time:.2f} 秒")

        end_time = time.time()
        self.history['total_training_time'] = end_time - start_time
        print("联邦学习训练完成!")
        self.print_cost_summary()
        self.save_detailed_history_to_txt(self.history, "fedavg-mnist20.txt")
...
def main():
    parser = argparse.ArgumentParser(description='联邦学习FedAvg实现')
    ...
    for i in range(args.num_clients):
        ...
        client = Client(i, train_loader, device, lr=args.lr)
        clients.append(client)

    server = Server(test_loader, device)
